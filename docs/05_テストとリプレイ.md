...
出撃中の記録（`Replay`）は毎コマンド `localStorage` に保存され、ページを閉じても次回起動時に `GameSession.replay()` で再生して途中から再開する（`tests/resume.test.ts`）。
リプレイの決定論がそのままセーブ機能になっているため、ルールを変えると古い記録は再生できなくなる。その場合は記録を捨てて拠点から始める。
