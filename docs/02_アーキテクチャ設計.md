...
## 4.7 効果音と BGM（`app/audio`）

音源ファイルは持たず、Web Audio API で合成する。ドメイン層は音を知らない。

| クラス | 役割 |
| --- | --- |
| `AudioEngine` | `AudioContext` の生成（最初のキー入力で `unlock()`）、単音 `tone()`・ノイズ `noise()` の合成、効果音／BGM のバス |
| `BgmPlayer` | 手続き的 BGM。テーマごとの音階・テンポ・コード進行から 16 分音符単位でベース・和音・メロディを先読み予約する |
| `SoundDirector` | `VisualEvent`（攻撃・ダメージ・回復・撃破・投擲…）、画面遷移（出撃・階移動・死亡・踏破・帰還）、UI（カーソル・開閉・決定）を効果音に写す |

```mermaid
flowchart LR
  GS[GameSession.visuals] --> GC[GameController.drainVisuals]
  GC --> AP[AnimationPlayer]
  GC --> SD[SoundDirector.onVisuals]
  APP[AppController 遷移] --> SD
  MAIN[main.ts キー入力] -->|open/close/cursor| SD
  SD --> AE[AudioEngine]
  BGM[BgmPlayer] --> AE
```

- BGM は場面（拠点＝`village`、ダンジョン＝テーマ ID）で切り替わる。
- 効果音・BGM の ON/OFF は拠点の「設定」で切り替え、`RenderSettings`（localStorage）に保存する。

## 4.8 ダンジョン途中のセーブ

出撃中の状態は保存しない。代わりに **リプレイ（seed＋持ち込み＋コマンド列）を毎コマンド保存** し、再起動時に再生して同じ状態を復元する。

| 項目 | 実装 |
| --- | --- |
| 保存先 | `BaseStorage.saveSortie()`（localStorage キー `mysterydungeon.sortie.v1`） |
| タイミング | `AppController.autosave()`。`session.history` が増えたときだけ書き込む（キー入力・ダッシュの各歩） |
| 復元 | `AppController.resumeSortie()`。`GameSession.replay()` で再生し、`status === 'playing'` なら出撃中の画面から始める |
| 破棄 | 帰還（`returnToBase`）で削除。再生に失敗した記録（ルール変更など）も削除して拠点から始める |

拠点データは出撃開始時点で保存済み（持ち物・仲間はセッション側にある）なので、復元はセッションだけでよい。
