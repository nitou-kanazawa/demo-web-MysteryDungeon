# アーキテクチャ設計書

## 1. レイヤー構成

```mermaid
flowchart TB
  subgraph app["src/app（プレゼンテーション層）"]
    main["main.ts<br/>起動・rAF ループ・キー配線"]
    ctrl["GameController<br/>UI 状態機械 → Command 変換"]
    render["render/*<br/>TileArt / SpriteArt / Lighting / Hud / Menu"]
  end
  subgraph domain["src/domain（ドメイン層・DOM 非依存）"]
    session["game/GameSession<br/>ファサード。Command 実行とターン進行"]
    exec["game/ActionExecutor<br/>移動・攻撃・撃破・仲間化"]
    effect["game/EffectResolver<br/>アイテム効果"]
    floor["game/FloorBuilder<br/>フロア生成・配置"]
    state["game/GameState<br/>可変状態のみ"]
    ai["ai/MonsterAI, AllyAI"]
    map["map/DungeonGenerator, DungeonMap,<br/>Visibility, Pathfinding"]
    item["item/ItemFactory, Inventory,<br/>PotService"]
    alchemy["alchemy/RecipeBook"]
    entity["entity/Player, Monster, Ally"]
    data["data/* マスターデータ"]
    core["core/Rng, Vec2, Id"]
  end
  main --> ctrl --> session
  main --> render
  render -. 読み取りのみ .-> state
  session --> exec & effect & floor & ai & item
  exec --> state
  floor --> map & data
  item --> alchemy & data
  session --> state
  domain --> core
```

### 依存ルール
- `app` → `domain` の一方向のみ。`domain` は `window`/`document` を参照しない。
- `render/*` は `GameState` を**読むだけ**。状態変更は必ず `GameSession.execute(Command)` を通す。
- `data/*` は純粋なオブジェクトリテラル（マスターデータ）。ロジックを持たない。

## 2. 主要クラス

| クラス | 責務 | SOLID 上の位置づけ |
| --- | --- | --- |
| `GameSession` | 1 プレイのファサード。`Command` を受け取り、ターンを進める | Facade。UI からの唯一の入口 |
| `GameState` | フロア・ターン・アクター・床上アイテムなどの可変状態 | データ保持のみ（SRP） |
| `ActionExecutor` | 移動・攻撃・ダメージ・撃破・仲間化の共通処理 | プレイヤーと AI が同じルールを共有（DRY） |
| `EffectResolver` | `ItemEffect`（判別共用体）をゲーム状態へ適用 | 新効果は union に追加＋分岐追加（OCP は限定的） |
| `FloorBuilder` | 地形生成→配置。出現テーブルの重み抽選 | 生成手順の集約 |
| `DungeonGenerator` | セクター分割方式の地形生成 | `GeneratorConfig` で差し替え可能 |
| `Visibility` | 部屋単位の視界と探索済み記憶 | 描画とロジックの両方が利用 |
| `PotService` | 壺 4 種のルール | 状態は `ItemInstance` に、ルールはサービスに（分離） |
| `RecipeBook` | レシピ検索（多重集合一致）と発見管理 | データ駆動 |
| `MonsterAI` / `AllyAI` | `AiAction` を返す純粋な判断 | 判断と実行の分離（`perform()` は Session 側） |
| `IRng` / `SeededRng` | 乱数抽象と決定論的実装 | DIP。テストで差し替え可能 |
| `ShopService` | 店の値札・支払い・売却・どろぼう | ルールを 1 か所に集約 |
| `Codex` | 図鑑の登録状況 | 拠点とセッションで同一インスタンスを共有 |
| `HomeBase` / `Campaign` | 出撃をまたぐ状態と、出撃⇄帰還の反映 | セッションと拠点の分離 |
| `BaseStorage` | 永続化先の抽象（localStorage / メモリ） | DIP |
| `SkillExecutor` | 特技の効果適用（対象選定は AI） | 判断と実行の分離 |
| `AllyAI` + `Tactic` | 作戦パラメータで挙動を切り替え | データ駆動の戦略 |

## 3. コマンドパターン

```mermaid
sequenceDiagram
  participant K as キー入力
  participant C as GameController
  participant S as GameSession
  participant E as ActionExecutor / EffectResolver
  participant N as MonsterAI / AllyAI
  K->>C: keydown
  C->>C: UiMode に応じて Command を組み立て
  C->>S: execute(Command)
  S->>S: history.push(Command)
  S->>E: dispatch（移動／攻撃／使用…）
  E-->>S: CommandResult{consumedTurn}
  alt consumedTurn
    S->>N: decide(ally) / decide(monster)
    N-->>S: AiAction
    S->>E: perform(AiAction)
    S->>S: endTurn()（満腹・回復・状態異常・壺・湧き）
  end
  S-->>C: CommandResult
```

- `Command` はアイテムを**実体ではなくインデックス**で参照する。これにより JSON 化とリプレイが可能。
- `GameSession.replay({seed, commands})` は新しいセッションを作って同じコマンド列を流すだけ。
  乱数消費順が同じである限り、完全に同一の状態になる（`tests/replay.test.ts` で回帰検証）。

## 4. グラフィック（ピクセルスプライト）

- `app/render/sprites/PixelSprite.ts`: 文字列の行（1 文字 = 1 ピクセル、`.` は透明）とパレットで表すスプライト。`recolor()` で形を共有して色違いを作る。
- `SpriteCache` がスプライトを倍率ごとにオフスクリーンへ焼き、以後は `drawImage`（最近傍拡大）だけ。
- `monsterSprites.ts` に全種族（12 種）、`heroSprite.ts` に主人公。`tests/sprites.test.ts` で全種族にスプライトがあり 16×16 でパレットが揃うことを検証。
- `SpriteArt.drawCreature()` は種族IDでスプライトを引き、無ければ旧来の丸い図案にフォールバックする。ダンジョン・拠点・牧場・図鑑で共通。
- 図鑑はモンスターをスプライト、アイテムを既存のアイコン描画で拡大表示する。

![sprites](images/sprites.png)

### 描画パイプライン

```mermaid
flowchart LR
  A["TileArt（キャッシュ）<br/>石畳・レンガ・階段"] --> Z["合成"]
  B["SpriteArt<br/>床上アイテム → アクター"] --> Z
  Z --> L["Lighting<br/>①闇で塗る ②探索済みを薄く抜く<br/>③視界を clip して松明を抜く<br/>④soft-light で暖色"]
  L --> H["HUD / ログ / メニュー"]
```

ライティングは `destination-out` で「闇の層」を削る方式。視界外へ光が漏れないよう視界タイルで `clip()` している。
松明の半径は `sin` 2 波の合成でゆらぐ。

## 5. ディレクトリ

```
src/
  domain/
    core/      Vec2, Rng, Id
    map/       Tile, Room, DungeonMap, DungeonGenerator, Pathfinding, Visibility
    entity/    Actor, Player, Monster, Ally, MonsterDef, StatusEffect
    item/      ItemDef, ItemInstance, ItemFactory, Inventory, PotService
    alchemy/   Recipe, RecipeBook
    ai/        AiAction, AiUtil, MonsterAI, AllyAI
    game/      GameState, GameSession, Command, ActionExecutor, EffectResolver, FloorBuilder, Combat, Placement, MessageLog,
               ShopService, Codex, SkillExecutor, Tactic
    base/      HomeBase, Campaign（出撃をまたぐメタ層）
    data/      monsters, items, recipes, spawnTables, skills, breeding
  app/
    AppController.ts   拠点 ⇄ ダンジョンのシーン切替・永続化
    GameController.ts
    base/      BaseController, BaseRenderer, BaseStorage
    input/KeyMap.ts
    ui/UiState.ts, ItemActionMenu.ts
    render/RenderConfig, TileArt, SpriteArt, Lighting, HudRenderer, MenuRenderer, Renderer, CodexRenderer
    render/sprites/PixelSprite, monsterSprites, heroSprite
    main.ts
tests/         Vitest（ドメイン層のみ。DOM 不要）
docs/          本ドキュメント群
```
